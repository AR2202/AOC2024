...
evalWireMap m s = case M.lookup s m of 
    ...
    Just (Or s1 s2) -> evalWireMap (M.insert  s (Lit (s1eval || s2eval)) mNew) s
        where s1eval = evalWireMap m s1
              ...
              m1 = M.insert s1 (Lit s1eval) m 
              mNew = M.insert s2 (Lit s2eval) m1 
    Just (Xor s1 s2) -> evalWireMap (M.insert  s (Lit (s1eval /= s2eval)) mNew) s
        where s1eval = evalWireMap m s1
              s2eval = evalWireMap m1 s2 
              m1 = M.insert s1 (Lit s1eval) m 
              mNew = M.insert s2 (Lit s2eval) m1 
...
evalAllZs m = map (evalWireMap m ) zkeys
    where zkeys = sort $ filter (isPrefixOf "z") $ M.keys m 
makeWireMap lits ops = foldl' (flip parseLineLit)
 (foldl' (flip parseLineOp) M.empty ops) lits
bool2Num False = 0
bool2Num True = 1 

toDecimal _ []  = 0 
toDecimal n (x:xs)  = x*(2^n) + toDecimal  (n+1) xs
day24ex :: IO ()
day24ex = do 
    ...
    print $ toDecimal 0$ map bool2Num $ evalAllZs m
